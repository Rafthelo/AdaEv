...
const StatCard = ({ icon, title, value, subtitle, color = 'text-gray-800' }) => (
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 px-6 py-4 mb-6 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          {activeEvents.length === 0 ? (
            <span className="text-sm text-gray-400">No hay eventos activos</span>
          ) : (
            <select
              value={selectedEvent}
        ...
        <p className="text-xs text-gray-400">Se actualiza cada 30 segundos</p>
      </div>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <StatCard
              ...
              color={stats.summary.low_stock > 0 ? 'text-red-600' : 'text-gray-400'}
            />
            <StatCard
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200">
              ...
              <div className="divide-y divide-gray-50">
                {stats.recent_sales.length === 0 ? (
                  <p className="px-6 py-4 text-sm text-gray-400">Sin ventas registradas</p>
                ) : stats.recent_sales.map((sale) => (
                  <div key={sale.id} className="px-6 py-3 flex items-center justify-between">
                    <div>
                      <p className="text-sm font-medium text-gray-800">#{sale.id} — {sale.seller_username || '—'}</p>
                      <p className="text-xs text-gray-400">{sale.product_codes || '—'} · {new Date(sale.created_at).toLocaleTimeString('es-BO')}</p>
                    </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200">
              <div className="px-6 py-4 border-b border-gray-100">
                <h3 className="font-semibold text-gray-800">Rendimiento por vendedor</h3>
              </div>
              <div className="divide-y divide-gray-50">
                {stats.seller_performance.length === 0 ? (
                  <p className="px-6 py-4 text-sm text-gray-400">Sin vendedores asignados</p>
                ) : stats.seller_performance.map((seller, i) => (
                  <div key={i} className="px-6 py-3 flex items-center justify-between">
                    <div>
                      <p className="text-sm font-medium text-gray-800">{seller.username}</p>
                      <Badge label={seller.seller_type_label} color={SELLER_TYPE_COLORS[seller.seller_type] || 'gray'} />
                    </div>
                    <div className="text-right">
                      <p className="text-sm font-bold text-green-600">Bs. {seller.total_revenue.toFixed(2)}</p>
                      <p className="text-xs text-gray-400">{seller.total_sales} ventas</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {stats.low_stock_items.length > 0 && (
            <div className="bg-white rounded-xl shadow-sm border border-red-200">
              <div className="px-6 py-4 border-b border-red-100">
                <h3 className="font-semibold text-red-700">⚠️ Productos con stock bajo</h3>
              </div>
